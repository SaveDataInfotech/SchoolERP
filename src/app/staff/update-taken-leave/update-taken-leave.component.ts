...
@Component({
  selector: 'app-update-taken-leave',
  templateUrl: './update-taken-leave.component.html',
  styleUrls: ['./update-taken-leave.component.scss']
})
export class UpdateTakenLeaveComponent implements OnInit {
  ...
  async dateChange() {
    ...
    for (var d = fromdate; d <= todate; d.setDate(d.getDate() + 1)) {
      ...
      if (student.length) {
        ...
        this.notificationSvc.warn('Attendance is not Put for the days you have selected');
        this.staffLeavePermissionForm.get('fromdate')?.setValue('');
        this.staffLeavePermissionForm.get('todate')?.setValue('');
      }
    }
      ...
      this.DialogSvc.openConfirmDialog('Are you sure want to add this record ?')
        .afterClosed().subscribe(res => {
          if (res == true) {
            ...
            this.slpSvc.takenNewPermission(formvalue).subscribe(res => {
  ...
  async pFNChange() {
    ...
    const newAttendance = student.filter((e) => { return e.fn == false && e.fni == true });
    debugger;
    if (FN == true) {
      if (newAttendance.length) {
        ...
        this.notificationSvc.warn('Attendance is not Put for the days you have selected OR You had present on thi day');
        this.staffHalfDayPermissionForm.get('h_leave_days')?.setValue('');
      }
    }
  }
  ...
  async pANChange() {
    ...
    debugger;
    const student = await this.slpSvc.checkAttendanceStaff(staffNo, this.datepipe.transform(leaveDays, 'yyyy-MM-dd')).toPromise();
    const newAttendance = student.filter((e) => { return e.an == false && e.ani == true });
      ...
      this.DialogSvc.openConfirmDialog('Are you sure want to add this record ?')
        .afterClosed().subscribe(res => {
          if (res == true) {
            ...
            this.slpSvc.takenNewHalfDay(formvalue).subscribe(res => {
